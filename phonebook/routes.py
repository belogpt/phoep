...
def new_contact():
    ...
    groups = repository.get_groups_with_counts()
    return render_template('edit_contact.html', contact=None, groups=groups)
...
def _save_contact(contact_id=None):
    ...
    contact = Contact(group=group, name=name, office=office, mobile=mobile, other=other, photo=photo, contact_id=contact_id)
    try:
